...
const (
	// Documents are stored as: githubusers:{username}.
	indexKey         = "githubusers"
	indexName        = "idx:githubusers"
	defaultSortField = "updated_at"
)

// Repository provides Redis-based persistence for GitHub users using RedisJSON and RediSearch.
...
func (r Repository) Exist(ctx context.Context, username github.Username) (bool, error) {
	builder := redisquery.
		NewBuilder().
		Field("username").
		Tag(string(username))
...
func (r Repository) List(ctx context.Context, limit, offset int, result *github.ListResult) error {
...
func (r Repository) Query(
	...
	*result = github.ListResult{
	...
	for _, user := range users {
		result.Entities = append(result.Entities, user.GetGithubUser())
	}
...
// Returns github.ErrUserAlreadyExist if the user already exists.
func (r Repository) Create(ctx context.Context, username github.Username, user *github.User) error {
	userModel := NewGithubUserModel(username)
	...
	if exist {
		return github.ErrUserAlreadyExist
	}
	...
	userModel.ToGithubUser(user)

	return nil
}

// Get retrieves a user by username.
// Returns github.ErrUserNotFound if the user does not exist.
func (r Repository) Get(ctx context.Context, username github.Username, u *github.User) error {
...
// Returns github.ErrUserNotFound if the user does not exist.
func (r Repository) Delete(ctx context.Context, username github.Username) error {
...
func (r Repository) userKey(u github.Username) string {
